...
```{r}
# Assuming data is your data frame and it's already loaded
...
set.seed(123)
train_index <- sample(1:nrow(nypd_project_data), nrow(nypd_project_data)*0.7)
train_set <- nypd_project_data[train_index,]
test_set <- nypd_project_data[-train_index,]

# Build the decision tree model
model <- rpart(BORO ~ VIC_SEX + VIC_AGE_GROUP + VIC_RACE + OCCUR_TIME, data = train_set, method = "class")

# Make predictions on the test set
predictions <- predict(model, test_set, type = "class")

# Evaluate the model
table(pred = predictions, true = test_set$BORO)
